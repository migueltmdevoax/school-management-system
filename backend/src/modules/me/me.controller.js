...
export const getMyDashboard = async (req, res, next) => {
  try {
    ...
    if (role === "teacher") {
      ...
      if (!teacherId) return res.status(400).json({ success: false, message: "teacher_id not found. Please log out and log in again." });
    ...
    if (role === "parent") {
      ...
      if (!parentId) return res.status(400).json({ success: false, message: "parent_id not found. Please log out and log in again." });
...
export const getMyStudents = async (req, res, next) => {
  try {
    const { role } = req.user;
    if (role === "admin") return res.status(400).json({ success: false, message: "Admins use /api/students" });
    if (role === "teacher") {
      const teacherId = req.user.teacher_id;
      if (!teacherId) return res.status(400).json({ success: false, message: "teacher_id not found. Please log out and log in again." });
...
// 🔥 Nuevo — hijos del parent para selectores en Alergias, Hitos, etc.
